...
# Decrease this value if you encount problems with your cpu/ram usage.
max = threading.Semaphore(value=5)
threads = []
...
def fetchData(channel='google', post='1', proxy=None):
        global postValid
    # try:
        r = requests.get('https://t.me/'+channel+'/'+post+'?embed=1',
                         verify=False, timeout=20, proxies={'https': proxy})
...
def run(channel, post, proxy):
    ...
    if (type(s) is dict):
        l = addViewToPost(
            channel, post, s['key'], s['cookie'], 'http://'+proxy)
        if l != False:
...
channelName = "sharique156789testing"
postStart = 2
postValid = True

for post in range(postStart, postStart + 3):
    if postValid == False:
        break
